-- ============================================================================= 
-- TASKFLOW AI - COMPLETE DATABASE SCHEMA WITH RELATIONSHIPS
...
CREATE TABLE IF NOT EXISTS public.tasks (
    ...
    is_archived BOOLEAN DEFAULT FALSE, -- New column for soft deletion
...
CREATE TABLE IF NOT EXISTS public.meeting_summaries (
    ...
    language VARCHAR(10) DEFAULT 'en-US', -- Speech recognition language
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- Add policies for is_archived column
CREATE POLICY "Users can view archived tasks" ON public.tasks
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can update archived tasks" ON public.tasks
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can view archived notes" ON public.notes
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can update archived notes" ON public.notes
    FOR UPDATE USING (auth.uid() = user_id);
