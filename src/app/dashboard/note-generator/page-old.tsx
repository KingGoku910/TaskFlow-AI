...
export default function NoteGeneratorPage() {
  ...
  const [pageMode, setPageMode] = useState<PageMode>('generator');
  ...
  const [editingNote, setEditingNote] = useState<SavedNote | null>(null);
  const [isConvertingToTasks, setIsConvertingToTasks] = useState(false);
  const [userId, setUserId] = useState<string | null>(null);
  const [isExporting, setIsExporting] = useState(false);
  ...
  const handleExportToPDF = async () => {
    ...
    try {
      ...
      tempDiv.style.paddingBottom = '60px'; // Add bottom margin
  ...
  const handleSaveNoteAsAdvanced = (note: SavedNote) => {
  ...
  const handleConvertNotesToTasks = async () => {
    if (!currentNote || !userId) return;

    setIsConvertingToTasks(true);
    try {
      const tasks = await convertNotesToTasks(currentNote.notes, currentNote.topic);
      
      // Add each task to the user's task list
      for (const task of tasks) {
        await addTaskDirect(userId, {
          title: task.title,
          description: task.description,
          priority: task.priority as 'low' | 'medium' | 'high',
          deadline: task.estimatedHours ? 
            new Date(Date.now() + task.estimatedHours * 60 * 60 * 1000).toISOString() : 
      ...
      toast({
        title: "Tasks Created",
        description: `${tasks.length} tasks created from your notes. Check your Tasks page.`,
        variant: "default"
      });
    } catch (error) {
      ...
      toast({
        ...
        description: "Failed to convert notes to tasks. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsConvertingToTasks(false);
    }
  };
  ...
  const handleNoteCreated = (note: SavedNote) => {
    setSavedNotes(prev => [note, ...prev]);
    toast({
      title: 'Note Created',
      description: `Note "${note.title}" has been created successfully.`,
    });
  };

  const handleNoteUpdated = (updatedNote: SavedNote) => {
    setSavedNotes(prev => prev.map(note => 
      note.id === updatedNote.id ? updatedNote : note
    ));
    toast({
      title: 'Note Updated',
      description: `Note "${updatedNote.title}" has been updated successfully.`,
    });
  };
  ...
  const handleUnarchiveNote = (noteId: string) => {
    ...
    toast({
      ...
      description: 'Note has been unarchived successfully.',
    });
  };

  const handleBulkArchive = (noteIds: string[]) => {
    setSavedNotes(prev => prev.map(note => 
      noteIds.includes(note.id) ? { ...note, archived: true } : note
    ));
    toast({
      title: 'Notes Archived',
      description: `${noteIds.length} notes have been archived successfully.`,
    });
  };

  const handleBulkUnarchive = (noteIds: string[]) => {
    setSavedNotes(prev => prev.map(note => 
      noteIds.includes(note.id) ? { ...note, archived: false } : note
    ));
    toast({
      title: 'Notes Unarchived',
      description: `${noteIds.length} notes have been unarchived successfully.`,
    });
  };
  ...
  const handleDuplicateNote = (note: SavedNote) => {
    ...
    setSavedNotes(prev => [duplicatedNote, ...prev]);
    toast({
  ...
  const handleBulkDeleteNotes = (noteIds: string[]) => {
    setSavedNotes(prev => prev.filter(note => !noteIds.includes(note.id)));
    toast({
      title: 'Notes Deleted',
      description: `${noteIds.length} notes have been deleted successfully.`,
    });
  };

  const handleBulkArchiveNotes = (noteIds: string[]) => {
    handleBulkArchive(noteIds);
  };

  const handleExportNotes = (noteIds: string[]) => {
    const notesToExport = savedNotes.filter(note => noteIds.includes(note.id));
    ...
    toast({
      ...
      description: `${noteIds.length} notes have been exported successfully.`,
    });
  };
  ...
  return (
    <div className="space-y-6 w-full h-full overflow-y-auto flex-1 flex flex-col">
      <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">
              ...
              <Card className="shadow-lg w-full">
                <CardHeader>
                  <DashboardPageHeader
                    ...
                    description="Generate structured notes with AI and refine them through conversation."
                    icon={<Wand2 />}
                  />
                </CardHeader>
            <CardContent className="space-y-4">
              {mode === 'generate' ? (
                <>
                  ...
                  {/* Format Selection in Generator Panel */}
                  <div className="space-y-2">
        ...
        <div className="space-y-6">
          <SavedNotesManager onCreateTask={handleCreateTaskFromNote} />
        </div>
      </div>
          ...
          <Card className="shadow-lg w-full">
            <CardHeader>
              <DashboardPageHeader
                ...
                description="Create and edit notes with AI assistance, voice recording, and image analysis."
                icon={<Edit3 />}
              />
            </CardHeader>
            <CardContent>
          ...
          <Card className="shadow-lg w-full">
            <CardHeader>
              <DashboardPageHeader
                ...
                description="Manage note archiving with automatic scheduling and bulk operations."
                icon={<Archive />}
              />
            </CardHeader>
            <CardContent>
          ...
          <Card className="shadow-lg w-full">
            <CardHeader>
              <DashboardPageHeader
                ...
                description="Convert your notes to speech with advanced voice controls and settings."
                icon={<Volume2 />}
              />
            </CardHeader>
            <CardContent>
              <TextToSpeechPlayer
                text={currentNote?.content || selectedNote?.content || ""}
                title={currentNote?.topic || selectedNote?.title || "No note selected"}
        ...
        <TabsContent value="manager" className="space-y-6">
          <Card className="shadow-lg w-full">
            <CardHeader>
              <DashboardPageHeader
                title="Note Manager"
                description="Visual management of your notes with advanced filtering and organization."
                icon={<Grid />}
              />
            </CardHeader>
            <CardContent>
              <IconThumbnailView
                ...
                onBulkArchive={handleBulkArchiveNotes}
                onExport={handleExportNotes}
                onConvertToTasks={convertNotesToTasks}
              />
            </CardContent>
          </Card>
        </TabsContent>
